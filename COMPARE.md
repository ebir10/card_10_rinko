...
### §10.1 結果(2026-08-30実施)

`results/stress_test/comparison_tilt_photo.json`,
`results/stress_test/comparison_light_photo.json` に生データを保存。

| 条件 | 古典CV accuracy | YOLOv8 accuracy |
|---|---|---|
| 通常(E) | 96.2% (50/52) | 100.0% |
| 傾き(合成) | 92.3% (48/52) | 100.0% |
| **傾き(実写)** | **75.0% (39/52)** | 100.0% |
| 照明変化(合成) | 75.0% (39/52), UNKNOWN 2件 | 100.0% |
| **照明変化(実写)** | **75.0% (39/52)**, UNKNOWN 0件 | 100.0% |

![難条件による正解率の変化(合成 vs 実写)](results/stress_test/accuracy_chart.png)

![サンプル画像 通常/傾き(合成・実写)/照明変化(合成・実写)](results/stress_test/samples_grid.png)

![難条件比較サマリー表(合成 vs 実写)](results/stress_test/summary_table.png)

**YOLOv8はここでもどの条件で100%を維持した。** 実写でも合成でも、
学習元データセットの多様性がそのまま効いている。

**古典CVは実写の方が合成より厳しい結果になった:**

- **傾き**: 合成(92.3%)→実写(75.0%)とさらに低下した。原因は撮影時の
  傾きが合成の想定(±10〜22°)より大きく、かつ**カード自体を手で傾けて
  撮影したため画面内回転(in-plane rotation)だけでなく、わずかな
  奥行き方向の傾き(遠近が生じる傾き)も混ざった**ためと考えられる。
  合成は`cv2.warpAffine`による純粋な画面内回転のみを再現しており、
  この奥行き方向の歪みは再現できていなかった。
  → **合成版は実際の傾きの厳しさを過小評価していた。**
- **照明変化**: 合成(75.0%, UNKNOWN 2件)と実写(75.0%, UNKNOWN 0件)は
  正解率こそ一致したが、内訳が異なる。合成は明るさを一律倍率で処理した
  ため2枚で背景/カード分離自体に失敗(UNKNOWN)したが、実写は同じ
  「明るめ照明」設定でも露出の効き方が場所ごとに不均一(中央が白飛びし、
  周辺は暗いまま、というムラ)で、UNKNOWNにはならないがランクを
  誤認識するケースが多かった。**合成は正解率の"当たり"は一致したが、
  失敗の中身(検出失敗 vs 誤判定)は実写と異なっていた。**

**まとめ**: 合成データは傾向を掴む一次スクリーニングとしては有用だったが、
実際の撮影条件(手ブレによる複合的な歪み、照明ムラ)は合成では再現しきれず、
実写の方が古典CVにとってより厳しい結果になった。この結果は、
「合成データだけで頑健性を評価するのは危険で、可能な限り実写データで
検証すべき」という教訓を裏付けている。
